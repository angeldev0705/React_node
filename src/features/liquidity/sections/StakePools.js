  ...
  return (
    <Grid container style={{paddingTop: '4px'}}>
      <Grid item xs={12}>
        ...
        {/*<h3 className={classes.secondTitle}>aaaa {t('Stake-Second-Title')}</h3>*/}
      </Grid>
      {/*
        fromPage == 'page' && 
        <Hidden xsDown>
            ...
            <TableBody classes={{
              ...
              {pools.length > 1  && pools.map((pool, index) => (
                <StyledTableRow key={pool.name} style={{display: Boolean(index !== 3) ? "none" : "table-row"}}>
                  ...
                  <StyledTableCell >{poolsInfo[index]}</StyledTableCell>
                  <StyledTableCell >{poolsInfo[index]}</StyledTableCell>
                  <StyledTableCell >{poolsInfo[index]}</StyledTableCell>
                  <StyledTableCell component="th" >
                    <CustomButtons
        ...
        {pools.map((pool, index) => (
          <div key={`mobile-${index}`} className={classes.mobileContainer} style={{display: Boolean(index !== 3) ? "none" : ""}}>
            ...
            <div className={classes.mobileDetail}>
              ...
              <div style={{marginBottom:'10px'}}>{t('Stake-Table-Staked')+": "+poolsInfo[index]}</div>
